...
const Login = () => {
  ...
  const sendForgotPassForm = async (e: { preventDefault: () => void }) => {
    ...
    try {
      const loginUser = await Axios.post(`${API_URL}/users/forgotpass`, {
      ...
      setForgotLinkSentStatus(data.forgotPassSent)

      if (data.forgotPassSent === 0) {
        return setForgotLinkMsg(data?.message)
      }

      const { _id, userEmail, userTel, token } = data

      //if user is logged in
      setForgotLinkMsg(data?.message)
      localStorage.setItem('user', JSON.stringify({ _id, userEmail, userTel, token }))

      navigate('/auth/login')
    } catch ({ response }) {
      setForgotLinkMsg(response?.data?.message)
    } finally {
      setloading(false)
    }
  }
